...
router.post('/login', (req, res, next) => {
  ...
  fs.readFile(path.join(__dirname, '../models/users.json'), 'utf-8', (err, data) => {
    if (err) return next(err); // Pass any error to the error handling middleware (500)
    ...
    if (user) {
      // If user exists, redirect to the dashboard with 302 (temporary redirect)
      return res.status(200).redirect('/api/dashboard'); // Success with redirect
    } else {
      // If user doesn't exist, redirect to the register page with 302
      return res.status(200).redirect('/api/register'); // Success with redirect
    }
  });
});
...
router.post('/register', (req, res, next) => {
  ...
  fs.readFile(path.join(__dirname, '../models/users.json'), 'utf-8', (err, data) => {
    ...
    fs.writeFile(path.join(__dirname, '../models/users.json'), JSON.stringify(users, null, 2), (err) => {
      if (err) return next(err); // Pass any error to the error handling middleware (500)
      res.status(200).redirect('/'); // Success with redirect to login page
    });
  });
});
